...
it('allows keyboard choices and closes only the popover on Escape', async () => {
    ...
    expect(screen.getByRole('radio', { name: 'Nursing Rooms' })).toHaveFocus()
...
it.each([false, true])(
    'validates the range input and closes only after a valid save (mobile=%s)',
    async (mobile) => {
        const user = setup(mobile)
        ...
        await user.type(range, '3200{Enter}')
        await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument())
        ...
        await user.click(screen.getByRole('button', { name: 'Searching Range 3.2km' }))
        const savedRange = screen.getByRole('spinbutton', { name: 'Range (meters)' })
        expect(savedRange).toHaveValue(3200)
        await user.clear(savedRange)
        await user.type(savedRange, '5000{Escape}')
        await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument())
        expect(screen.getByRole('button', { name: 'Searching Range 3.2km' })).toHaveFocus()
        expect(JSON.parse(localStorage.getItem(PREFERENCES_KEY)!)).toMatchObject({ radius: 3200 })
    },
