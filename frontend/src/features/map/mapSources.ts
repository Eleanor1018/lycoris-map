...
/** Available providers in language order; a failed source is never retried in the same round. */
export function mapSourceOrder(language: Language): MapSource[] {
    const order: MapSource[] =
        language === 'zh' ? ['tencent', 'tianditu', 'osm'] : ['osm', 'tencent', 'tianditu']
    return order.filter(isMapSourceAvailable)
}
