...
## Crate 候选（crates.io API，未编译）

| Crate | 候选版本 | Crate | 候选版本 |
| --- | --- | --- | --- |
...
| tower-sessions | 0.15.0 | sha2 | 0.11.0 |
| bcrypt | 0.19.3 | image | 0.25.10 |
| tracing | 0.1.44 | tracing-subscriber | 0.3.23 |

来源：crates.io API。以上均为候选，尚未编译验证；实际采用版本由阶段实现时锁定。本轮认证不降级 `tower-sessions`，其具体适配设计由温晓在下一阶段确定。
