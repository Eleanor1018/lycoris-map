...
| 4：发布演练 | 已验收 | Linux 216 项、真实 TCP 43 路由/64 断言及 7 项超时检查通过；Web/Android、PG17→18→17、保留 Rust 新账号/译文/媒体的 Java 回退通过；十组配对性能三轮测量，正常负载零错误 |
| 5：空间查询优化 | 已验收 | 41 项小门禁、独立 7 项空间/迁移、Linux 全套 223 项、真实 TCP 43 路由/64 断言通过；三种规模 198 份执行计划与参数化查询验证、迁移后原 Java JAR 读写回退、Web/Android 查询复查通过 |
...
阶段 3 交付时的后续范围为 Linux/文件权限、构建容器、客户端、资源测量、已有库接管、重新登录切换、保留新增数据的 Java 回退与数据库升级回退。这些阶段 4 工作已完成，实际结果见下文；PostGIS 属阶段 5，认证最终形态仍单独设计。
...
### 阶段 4：最终验收与交付（2026-09-14）

- 补充矩阵在 Rust g6 新建点位 5005、账号并改密、真实 en 译文 10001、收藏、头像和点位图片。原 Java 在 PG18 g7 与最新 dump 恢复后的 PG17 g8 均实际登录该账号、读取中英文内容、核对身份/图片 URL 与 SHA-256，并继续写入点位 5006/5007。温晓独立严格复核 8 份原报告通过；全部旧代次 Cookie 重放为 401。
- PostgreSQL 18 最新原样 dump 恢复到 PG17.11 时，六张业务表、序列与 158 个媒体文件指纹一致；未剥离 dump 内容。Java 随后在 PG17 back 新增点位 5007，故恢复时点一致与后续 up/back 有意产生差异分别记录。最终活动库选择 PG18 up，保留 5006 个点位与 22 个账号。
- 十组正式性能测量使用最新快照 `perf-base-20260914`，每组前恢复同一 PG18 up 与媒体：idle 每组 3×15s；读缓存 on/off、登录、上传各预热 30s 后 3×20s。相同 2 CPU/512 MiB/UID10001、PG 池上限 10、BCrypt 成本 10；读/登录并发 8、上传并发 1。原始请求延迟、错误、资源采样与 PG 连接数保留，全部正常负载零错误。
- 温晓从十份原报告重新校验参数、数据/媒体/点位集合 hash、每轮计数、非零有效采样及错误率，并独立汇总内存。进程 RSS 有效样本加权均值：空闲 Java/Rust 为 325.347/13.986 MiB，缓存开读 416.672/26.858，缓存关读 415.171/27.520，登录 382.146/14.891，上传 383.073/17.147。差异约 93%～96%，仅代表这套实现和本地合成负载；同机其他用户 Docker 项目保持运行，不能推广为语言或生产的必然比例。
- 11 次配对恢复的指纹与媒体检查均通过；最终恢复后入口 Rust PG18 g20、缓存 on。温晓再次真实 HTTP 读取点位 5005 的 zh/en 精确标题通过，Web 用户 2 的签名/收藏保留。独占 UID10001 Linux 卷互读互写与非世界可写权限通过。
- 汇总工具修正重复导入旧汇总和损坏输入静默跳过；苏瑶纯 Python 54 项通过，温晓独立复跑新增 3 项通过。工具为展示汇总，最终完整性由上述十份原报告的独立检查确认。发布产品镜像保持阶段 4 已验收的 `51776d6b…`，后续性能工具与文档未重建产品。
- 原始证据归档至本地交付目录 `outputs/rust-migration-verification/stage4/`，共 76 份文件、逐文件 SHA-256 清单。包含 Linux/API/客户端、补充回退、十组性能与切换/恢复报告，以及温晓的独立复核；不包含私有备份、凭据、Cookie vault。
- Linux 216、真实 TCP 43/64 与额外 7 项超时检查、Web/Android 验收沿用本阶段已通过记录。Android 占位第三方底图未加载、未发起外部导航的边界保留。Docker stopping 与磁盘空间事故已解决，失败记录保留，不以旧失败轮次充当通过。

阶段 4 至此完成本地验收；本次文档提交连同已验收改动推送 `refactor/rust-backend`，准确提交及远端一致性写入本地 `stage4-delivery.json`。生产部署、生产数据库升级及认证重设计均未执行。

### 阶段 5：PostGIS 查询与兼容迁移最终验收（2026-09-14）

- 实现 `0002_spatial.sql` STORED geography 与公开已审核点位的 GiST、有限异常历史坐标部分索引；
  旧 `0001` 原文未改。附近查询先选保守候选，再按原半径 6371000 米的 Haversine 精确筛选及
  distance/id 排序，缓存升级 `nearby:v2`；保留原 DTO、语言、权限与视口接口。
- 单独记录两项修复：NaN/Infinity 不再使附近请求失败；高纬度旧包围盒漏点现在按产品球面距离
  返回。日期变更线、极点、有限越界、半径边界、排序、可见性和生成列同步均有真实 PG 测试。
- 苏瑶空间小门禁 41 项通过，温晓独立复跑新增空间/迁移 7 项通过；三规模正式测量保留 198 份
  完整执行计划。温晓逐份核对索引、结果/顺序和统计，并对 10 万点位补做参数化查询 11 次计划，
  默认 plan_cache_mode 下实际选择 custom plan 11 次，未把未选中的 generic plan 声称通过。
- 10 万点位热查询中位数（新/旧 SQL，ms）：城区 1 km 6.180/13.459、城区 50 km 6.105/14.247、
  稀疏区域 0.119/11.971。正常数据 IDs/顺序一致，两索引真实入选，未强制关闭 seqscan。
  首次执行不总更快，100k/50km 新查询 15.641 ms 略高于旧查询 15.385 ms；不将 SQL 计时当成
  整体 HTTP 性能或生产承诺。详见 [空间验收](stage5-spatial.md)。
- 整合后 Windows 在线 SQLx 元数据核对与格式/Clippy 通过；Linux 全量单次 **223 项通过、0 失败、
  0 跳过**，运行加固通过。温晓独立克隆合成测试库，对最终镜像完成 **64/64 TCP、43/43 模板**，
  克隆已清除。最终 imageId `sha256:d074e39c…`、binary `ea8deb49…`，保留阶段 4 镜像。
- 最新合成 PG18 在唯一完整快照后迁移，六表显式业务指纹、序列、158 个媒体文件完全一致。
  实际 Rust g21 → 原 Java g22 → Rust g23，旧代次 Cookie 全部 401。原 Java JAR 对新结构通过
  真实 HTTP 创建、编辑、审核、删除；保留点 5007 可由最终 Rust 附近查询读取。新账号改密、5005
  的 zh/en 译文、头像/点位图片、收藏和 Web 用户2签名均保留。温晓核对 12 份成功报告、3 份工具
  失败记录，并重新进行 HTTP/SQL/媒体检查通过，详见 [Java 兼容记录](stage5-java-compatibility.md)。
- Web/Android 使用现有源码与 APK 实际筛选、搜索、看图、跳转地图通过，详见
  [客户端复查](stage5-clients.md)。Android 第三方测试底图为空的限制保留，不记为通过。
- 已验收改动提交：空间实现 `9eb6065`、三规模测量 `35972fa`、Linux 发布 `a130bd0`、Java 兼容
  工具及实际演练 `d4c5938`。后续工具/文档不改变已验收二进制，无须重复全部发布测试。

阶段 5 至此完成本地验收。证据归档于本地任务 `outputs/rust-migration-verification/stage5/`，含
逐文件 SHA-256 清单；最终交付提交推送 `refactor/rust-backend` 并用 `git ls-remote` 核对，精确 SHA
记入 `stage5-delivery.json`。本轮没有生产部署、生产数据库升级或认证重设计。
