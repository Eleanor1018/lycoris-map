...
后续 Cargo 命令均在 `backend/` 内运行；仅传 `--manifest-path` 不会让 rustup 从目标
...
日常使用 Cargo 原生启动。在 `backend/` 内先设置 `DATABASE_URL`、`REDIS_URL`、`WRITE_ALLOWED_ORIGINS` 与 `SQLX_OFFLINE=true`，Windows 与 macOS/Linux 的完整示例见 [根 README](../README.md#克隆与初始化)。Rust 二进制只读取进程环境变量，不自动加载 `.env`；完整配置见 `.env.example`。相对 `UPLOAD_DIR` 从当前 crate 目录解析。
...
已有的本地 `scripts/run-local.py` 保留在磁盘，可继续从仓库根执行 `python backend/scripts/run-local.py`，按原有规则加载自己的 `.env`；该开发工具已被 Git 忽略，新拉取仓库使用上面的原生命令。
...
```powershell
# 在 backend 目录一步完成：迁移合成库、校验 .sqlx、离线构建、fmt / clippy / test
pwsh ./scripts/check-rust.ps1

# 或在 backend 目录直接运行（合成服务已启动并迁移）
$env:SQLX_OFFLINE = "true"
...
```powershell
# 在 backend/ 内运行；构建 release 运行镜像与独立 test stage
...
- 普通启动只做只读校验，确认所需迁移已应用且校验和一致，**不会自动执行 DDL**；
  空库需在 `backend/` 内显式运行 `cargo run --locked -- --migrate`。
