...
The subsequent range refinement replaces preset buttons and Apply with a single native number-pad field and an m suffix. Fresh preferences default to 1000m; saved preferences remain intact. A separate text draft commits on Done, loss of focus or sheet dismissal, so intermediate keystrokes never change the saved radius. Invalid/empty values restore the previous value; the existing range hint appears only while editing invalid input. Eight preference/link unit tests and the updated native settings flow passed, covering restart persistence, keyboard Done, invalid/empty edits and swipe-dismiss saving. The single-row Chinese simulator screenshot was inspected. Log: `/tmp/lycoris-single-range.log`.
