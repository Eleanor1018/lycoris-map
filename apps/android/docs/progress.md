...
The final compatibility build passed unit tests, QA lint and all four APK outputs in 1m50s. A new API 36 local suite passed 30 cases (one expected skip), and API 37 passed 29 (two expected skips). A later CI run exposed background map errors overwriting the speech fallback; this was fixed with action-notice priority and four deterministic regressions. Both live QA runners passed after Android 17 local-network permission handling. The new minimum-API CI job still requires a successful remote run. See [the detailed compatibility record](compatibility-2026-09-20.md).
